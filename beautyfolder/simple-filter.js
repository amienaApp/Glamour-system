...
function applyFilters() {
    ...
    const productGrid = document.getElementById('beauty-products-grid') || 
    ...
    // Debug: Check size filtering
    console.log('Size filtering debug:');
    products.forEach((product, index) => {
        if (index < 3) { // Only log first 3 products
            ...
            console.log(`Product ${index + 1} sizes:`, productSizes);
            console.log(`Product ${index + 1} selected sizes:`, productSelectedSizes);
            
            // Try to parse the JSON
            try {
                const parsedSizes = JSON.parse(productSizes || '[]');
                const parsedSelectedSizes = JSON.parse(productSelectedSizes || '[]');
                console.log(`Product ${index + 1} parsed sizes:`, parsedSizes);
                console.log(`Product ${index + 1} parsed selected sizes:`, parsedSelectedSizes);
            } catch (e) {
                console.log(`Product ${index + 1} JSON parse error:`, e);
            }
        }
    });
...
function updateProductCount(count) {
    ...
    if (countElement) {
        countElement.textContent = `${count} Beauty Products`;
    }
}
