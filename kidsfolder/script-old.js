...
// Filter Functionality - Define before DOMContentLoaded
    ...
    console.log('Clear filters button found:', !!clearFiltersBtn);
        ...
        console.log('Applying filters:', filterState);
            ...
            kids_categories: filterState.kids_category,
            gender: filterState.gender,
            age_group: filterState.age_group,
        ...
        console.log('Sending filter data:', filterData);
            ...
            if (data.success) {
                ...
                // Don't reload colors automatically to prevent infinite loops
                // Colors are loaded once on page load and remain stable
            } else {
        ...
        function createMediaElement(src, alt, isActive = false) {
            if (isVideoFile(src)) {
                return `<video src="../${src}" alt="${alt}" class="${isActive ? 'active' : ''}" data-color="${product.color}" muted loop playsinline style="width: 100%; height: 100%; object-fit: cover;">
                    Your browser does not support the video tag.
                </video>`;
            } else {
                return `<img src="../${src}" alt="${alt}" class="${isActive ? 'active' : ''}" data-color="${product.color}">`;
            }
        }
            ...
            card.innerHTML = `
                <div class="product-image">
                    ...
                    ${frontImage ? createMediaElement(frontImage, `${product.name} - Front`, true) : ''}
                    ${backImage && backImage !== frontImage ? createMediaElement(backImage, `${product.name} - Back`) : ''}
                    </div>
    ...
    function initializeProductCard(card) {
        ...
        function updateStyleCount(count) {
            ...
            styleCountElement.textContent = `${count} Kids Products`;
            }
        }
    ...
    // Function to check if only color filters are being applied
    function isOnlyColorFilter() {
        const hasColorFilter = filterState.color.length > 0;
        const hasOtherFilters = filterState.kids_category.length > 0 || 
                               filterState.gender.length > 0 ||
                               filterState.age_group.length > 0 ||
                               filterState.size.length > 0 || 
                               filterState.price_range.length > 0 || 
                               filterState.sub_subcategory.length > 0;
        
        return hasColorFilter && !hasOtherFilters;
    }
...
// Color loading is now only done once on page load

// Function to load dynamic colors from products
let isLoadingColors = false;
function loadDynamicColors() {
    const colorFilter = document.getElementById('color-filter');
    if (!colorFilter || isLoadingColors) return;
    ...
    // Build API URL
    let apiUrl = 'get-colors-api.php';
    if (currentSubcategory) {
        apiUrl += `?subcategory=${encodeURIComponent(currentSubcategory)}`;
    }
    
    // Show loading state
    colorFilter.innerHTML = '<div class="loading-colors">Loading colors...</div>';
    ...
    fetch(apiUrl)
        ...
        .then(data => {
            if (data.success && data.colors && data.colors.length > 0) {
                renderColorOptions(data.colors);
            } else {
                colorFilter.innerHTML = '<div class="no-colors">No colors available</div>';
            }
            isLoadingColors = false;
        })
        .catch(error => {
            console.error('Error loading colors:', error);
            colorFilter.innerHTML = '<div class="error-colors">Error loading colors</div>';
            isLoadingColors = false;
        });
}
...
function renderColorOptions(colors) {
    ...
    colors.forEach(colorData => {
        ...
        const borderStyle = color.toLowerCase() === '#ffffff' || color.toLowerCase() === '#fff' ? 'border: 1px solid #ccc;' : '';
        
        html += `
            <label class="color-option">
                <input type="checkbox" name="color[]" value="${color}" data-filter="color">
                <span class="color-swatch" style="background-color: ${color}; ${borderStyle}"></span>
            </label>
        `;
    });
    ...
    // Color options will use the main filter change handler
    // No need for separate event listeners
}

// Color event handling is now managed by the main filter change handler
// No separate color event listeners needed
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Load dynamic colors from products
    loadDynamicColors();
        ...
        if (e.target.classList.contains('color-circle')) {
            ...
            if (productCard) {
                ...
                images.forEach(media => {
                    if (media.getAttribute('data-color') === color) {
                        media.classList.add('active');
                        // If it's a video, play it
                        if (media.tagName === 'VIDEO') {
                            media.play().catch(e => console.log('Video play failed:', e));
                        }
                    } else {
    ...
    // Initialize all existing videos on page load
    function initializeAllVideos() {
        const allVideos = document.querySelectorAll('.image-slider video');
        console.log('Found videos to initialize:', allVideos.length);
        ...
        allVideos.forEach(video => {
            ...
            // Try to play the video if it's active
            if (video.classList.contains('active')) {
                const playPromise = video.play();
                if (playPromise !== undefined) {
                    playPromise.catch(error => {
    ...
    setTimeout(initializeAllVideos, 100);
