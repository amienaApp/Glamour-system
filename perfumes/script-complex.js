// Unified Perfumes Script
document.addEventListener('DOMContentLoaded', function() {
    console.log('Perfumes script loaded successfully');
    ...
    function initializeQuickView() {
        ...
        // Test if we can find quick view buttons
        const quickViewButtons = document.querySelectorAll('.quick-view');
        console.log('Found quick view buttons:', quickViewButtons.length);
        if (quickViewButtons.length > 0) {
            console.log('First quick view button:', quickViewButtons[0]);
            console.log('First button data-product-id:', quickViewButtons[0].getAttribute('data-product-id'));
        } 
    ...
    document.addEventListener('click', function(e) {
        ...
        if (e.target.classList.contains('quick-view') || e.target.closest('.quick-view')) {
            ...
            if (productId) {
                console.log('Calling openQuickView...');
            ...
            fetch(`../get-product-details.php?product_id=${productId}`)
                ...
                .then(data => {
                    ...
                    if (data.success && data.product) {
                        console.log('Product data loaded successfully:', data.product);
                ...
                .catch(error => {
                    ...
                    if (productCard) {
                        console.log('Using fallback method with product card data');
                        loadProductDataFromCard(productCard, productId);
                    } else {
                        console.error('No product card found for fallback');
                        closeQuickView();
                    }
                });
        } catch (error) {
    ...
    function loadProductDataFromCard(productCard, productId) {
        console.log('Loading product data from card for ID:', productId);
        
        // Get basic info from product card
        ...
        // Use fallback sizes for perfumes
        const sizes = ['30ml', '50ml', '100ml', '200ml'];
        setQuickViewSizes(sizes);
        
        // Handle images from product card
        ...
        if (mainImage && thumbnailsContainer) {
            ...
            if (productImage) {
                ...
                mainImage.alt = productImage.alt || name;
    ...
    function loadProductData(product) {
        console.log('Loading product data from API:', product);
        ...
        if (priceEl) priceEl.textContent = product.displayPrice || '$0';
        if (addToBagBtn) addToBagBtn.setAttribute('data-product-id', product.id);
        
        // Handle sizes from API data
        let sizes = [];
        let selectedQuickViewSize = '';
        
        console.log('Product sizes from API:', product.sizes);
        console.log('Product sizes type:', typeof product.sizes);
        console.log('Product sizes length:', product.sizes ? product.sizes.length : 'undefined');
        
        if (product.sizes && product.sizes.length > 0) {
            // Use sizes from API data
            sizes = product.sizes.map(size => typeof size === 'string' ? size : size.name);
            selectedQuickViewSize = sizes[0]; // Set first size as default
            console.log('Found sizes from API:', sizes);
        } else {
            // Use perfume-specific sizes as fallback
            console.log('No sizes found in API data - using perfume-specific sizes');
            sizes = ['30ml', '50ml', '100ml', '200ml'];
            selectedQuickViewSize = '100ml'; // Set default size
        }
        
        // Set the sizes in the quickview
        setQuickViewSizes(sizes);
        
        // Handle images from API data
        const mainImage = document.getElementById('quick-view-main-image');
        const thumbnailsContainer = document.getElementById('quick-view-thumbnails');
        
        if (mainImage && thumbnailsContainer && product.images && product.images.length > 0) {
            // Set main image from API data
            const firstImage = product.images[0];
            if (firstImage.type === 'video') {
                    const mainVideo = document.getElementById('quick-view-main-video');
                    if (mainVideo) {
                    mainVideo.src = firstImage.src;
        ...
        // Handle colors from API data
        if (product.colors && product.colors.length > 0) {
            setQuickViewColors(product.colors);
        }
        ...
        if (reviewCountEl && product.reviewCount) {
            reviewCountEl.textContent = `(${product.reviewCount} reviews)`;
        }
    }
    
    function setQuickViewSizes(sizes) {
        console.log('Setting quickview sizes:', sizes);
        
        const sizeSelection = document.getElementById('quick-view-size-selection');
        if (!sizeSelection) {
            console.error('Size selection element not found');
            return;
        }
        ...
        if (sizes.length === 0) {
            console.log('No sizes to display');
            return;
        }
        
        // Create size selection header
        ...
        sizeCount.textContent = `${sizes.length} sizes available`;
        ...
        sizes.forEach((size, index) => {
            ...
            sizeBtn.addEventListener('click', function() {
                ...
                // Store selected size
                selectedQuickViewSize = size;
                console.log('Selected size:', selectedQuickViewSize);
            });
    ...
    function setQuickViewColors(colors) {
        console.log('Setting quickview colors:', colors);
        
        const colorSelection = document.getElementById('quick-view-color-selection');
        if (!colorSelection) {
            console.error('Color selection element not found');
            return;
        }
        ...
        colors.forEach((color, index) => {
                ...
                border: 2px solid ${index === 0 ? '#000' : '#ddd'};
                        ...
                        z-index: 10;
                    `;
            colorBtn.title = color.name || color;
                    
            if (index === 0) {
                        colorBtn.classList.add('active');
                selectedQuickViewColor = color.color || color;
                    }
                ...
                selectedQuickViewColor = color.color || color;
                console.log('Selected color:', selectedQuickViewColor);
            });
    ...
    // Color circle functionality
            ...
            // If still no sizes, use perfume-specific sizes as fallback
            if (sizes.length === 0) {
                console.log('No sizes found for this product - using perfume-specific sizes');
                sizes = ['30ml', '50ml', '100ml', '200ml'];
                selectedQuickViewSize = '100ml'; // Set default size
            }
