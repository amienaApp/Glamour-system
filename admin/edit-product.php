...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (isset($_POST['color_variants']) && is_array($_POST['color_variants'])) {
            foreach ($_POST['color_variants'] as $index => $variant) {
                ...
                if (isset($_FILES['color_variants']['name'][$index]['front_image']) && 
                    ...
                    $frontImageName = time() . '_variant_' . $index . '_front_' . $_FILES['color_variants']['name'][$index]['front_image'];
                ...
                $productData['color_variants'][] = $variantData;
            }
        }
    ...
    else if (isset($_POST['products']) && is_array($_POST['products'])) {
...
<body>
    ...
    <div class="main-content">
        <div class="container">
            ...
            <p>Update product details, variants, and images</p>
        </div>
        ...
        <?php if (isset($product) && $product): ?>
            <div style="background: yellow; padding: 10px; margin: 10px 0; border: 2px solid red;">
                <strong>DEBUG - Product Data:</strong><br>
        ...
        <form method="POST" enctype="multipart/form-data" id="single-product-form" class="single-product-form form-active">
            ...
            <div class="form-group">
                            ...
                            <input type="number" id="price" name="price" value="<?php echo htmlspecialchars($product['price'] ?? ''); ?>" step="0.01" min="0" required placeholder="0.00">
            </div>
                ...
                <div class="image-inputs">
                    ...
                    <label for="front_image">Front Media *</label>
                        ...
                        <div id="front-preview" class="image-preview">
                                <?php if (isset($product['front_image']) && !empty($product['front_image'])): ?>
                                    <div class="existing-image">
                                        <img src="../<?php echo htmlspecialchars($product['front_image']); ?>" alt="Front" style="max-width: 200px; max-height: 200px;">
                                        <p>Current: <?php echo basename($product['front_image']); ?></p>
                                    </div>
                            <?php else: ?>
        ...
        <?php endif; ?>
                    ...
                    <label for="back_image">Back Media *</label>
                        ...
                        <div id="back-preview" class="image-preview">
                                <?php if (isset($product['back_image']) && !empty($product['back_image'])): ?>
                                    <div class="existing-image">
                                        <img src="../<?php echo htmlspecialchars($product['back_image']); ?>" alt="Back" style="max-width: 200px; max-height: 200px;">
                                        <p>Current: <?php echo basename($product['back_image']); ?></p>
                                    </div>
                            <?php else: ?>
                                <div class="no-image">No media selected</div>
<?php endif; ?>
                    ...
                    <div class="form-section">
                        ...
                        <div class="color-variants-section">
                            ...
                            <?php if (isset($product['color_variants']) && is_array($product['color_variants'])): ?>
                                <?php foreach ($product['color_variants'] as $index => $variant): ?>
                                    <div class="color-variant-item" data-variant-index="<?php echo $index; ?>">
            ...
            <div class="form-group">
                                                ...
                                                <input type="text" name="color_variants[<?php echo $index; ?>][name]" value="<?php echo htmlspecialchars($variant['name'] ?? ''); ?>" placeholder="e.g., Red, Blue, Black">
                </div>
            
            <div class="form-group">
                                                <label>Color</label>
                                                <input type="color" name="color_variants[<?php echo $index; ?>][color]" value="<?php echo htmlspecialchars($variant['color'] ?? '#000000'); ?>">
            </div>
            
            <div class="form-group">
                                                <label>Price</label>
                                                <input type="number" name="color_variants[<?php echo $index; ?>][price]" value="<?php echo htmlspecialchars($variant['price'] ?? ''); ?>" step="0.01" min="0" placeholder="0.00">
            </div>
            
                                            <div class="form-group">
                                                <label>Stock</label>
                                                <input type="number" name="color_variants[<?php echo $index; ?>][stock]" value="<?php echo htmlspecialchars($variant['stock'] ?? '0'); ?>" min="0" placeholder="0">
            </div>

                                            <div class="form-group">
                                                <label>Front Image</label>
                                                <input type="file" name="color_variants[<?php echo $index; ?>][front_image]" accept="image/*,video/*,.mp4,.webm,.mov,.avi,.mkv">
                                                <?php if (isset($variant['front_image']) && !empty($variant['front_image'])): ?>
                                                    <div class="existing-image">
                                                        <img src="../<?php echo htmlspecialchars($variant['front_image']); ?>" alt="Front" style="max-width: 100px; max-height: 100px;">
                                                        <p>Current: <?php echo basename($variant['front_image']); ?></p>
                                                    </div>
                                                        <?php endif; ?>
                                                </div>
                                            ...
                                            <div class="form-group">
                                                ...
                                                <input type="file" name="color_variants[<?php echo $index; ?>][back_image]" accept="image/*,video/*,.mp4,.webm,.mov,.avi,.mkv">
                                                <?php if (isset($variant['back_image']) && !empty($variant['back_image'])): ?>
                                                    <div class="existing-image">
                                                        <img src="../<?php echo htmlspecialchars($variant['back_image']); ?>" alt="Back" style="max-width: 100px; max-height: 100px;">
                                                        <p>Current: <?php echo basename($variant['back_image']); ?></p>
                                                    </div>
                                                        <?php endif; ?>
                                                </div>
                                            </div>
            ...
            <div class="form-group">
                <label for="description">Description</label>
                    ...
                    <div class="form-group" id="salePriceGroup" style="display: none;">
                        ...
                        <input type="number" id="salePrice" name="salePrice" value="<?php echo htmlspecialchars($product['sale_price'] ?? ''); ?>" step="0.01" min="0" placeholder="Enter sale price">
                        </div>
    ...
    <script>
        ...
        // Initialize form on page load/refresh
        document.addEventListener('DOMContentLoaded', function() {
            toggleSalePrice();
            
            // Only clear form if we're not editing an existing product
            <?php if (!isset($product) || !$product): ?>
                clearForm();
            <?php endif; ?>
            
            // Initialize existing size selection if product has sizes
            <?php if (isset($product['size_category']) && !empty($product['size_category']) && $product['size_category'] !== 'none'): ?>
                // Show size selection group
                document.getElementById('size_selection_group').style.display = 'block';
                
                // Load size options for the selected category
                loadSizeOptions();
                
                // Set selected sizes if they exist
                <?php if (isset($product['selected_sizes']) && !empty($product['selected_sizes'])): ?>
                    const selectedSizesArray = '<?php echo $product['selected_sizes']; ?>'.split(',');
                    selectedSizesArray.forEach(size => {
                        if (size.trim()) {
                            selectedSizes.add(size.trim());
                        }
                    });
                    updateSelectedSizesDisplay();
                <?php endif; ?>
            ...
            // Initialize sale price display if product is on sale
            <?php if (isset($product['sale']) && $product['sale']): ?>
                document.getElementById('salePriceGroup').style.display = 'block';
            <?php endif; ?>
            
            // Initialize subcategory if product has one
            <?php if (isset($product['category']) && !empty($product['category'])): ?>
                // Load subcategories for the existing category
                loadSubcategories();
                
                // Set the selected subcategory after a short delay to ensure subcategories are loaded
                <?php if (isset($product['subcategory']) && !empty($product['subcategory'])): ?>
                    setTimeout(function() {
                        const subcategorySelect = document.getElementById('subcategory');
                        if (subcategorySelect) {
                            subcategorySelect.value = '<?php echo htmlspecialchars($product['subcategory']); ?>';
                            // Trigger subcategory change handler to show any category-specific fields
                            handleHomeLivingSubcategory();
                            // Load sub-subcategories for Beauty & Cosmetics
                            loadSubSubcategories();
                            
                            // Set sub-subcategory value if it exists
                            <?php if (isset($product['sub_subcategory']) && !empty($product['sub_subcategory'])): ?>
                                setTimeout(function() {
                                    const subSubcategorySelect = document.getElementById('sub_subcategory');
                                    if (subSubcategorySelect) {
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            form.addEventListener('submit', function() {
                // Only clear form if we're adding a new product, not editing
                <?php if (!isset($product) || !$product): ?>
        ...
        function generateMultiBeautySizes(productIndex) {
            return `
                <div class="size-category">
                    ...
                    <div class="size-options">
                        <label><input type="checkbox" value="Sample" onchange="toggleMultiSizeSelection('${productIndex}', 'Sample')"> Sample (1-2ml)</label>
                        <label><input type="checkbox" value="Travel" onchange="toggleMultiSizeSelection('${productIndex}', 'Travel')"> Travel Size (5-10ml)</label>
                        <label><input type="checkbox" value="Regular" onchange="toggleMultiSizeSelection('${productIndex}', 'Regular')"> Regular (15-30ml)</label>
                        <label><input type="checkbox" value="Large" onchange="toggleMultiSizeSelection('${productIndex}', 'Large')"> Large (50-100ml)</label>
                        <label><input type="checkbox" value="Jumbo" onchange="toggleMultiSizeSelection('${productIndex}', 'Jumbo')"> Jumbo (150ml+)</label>
                    </div>
                </div>
                ...
                <div class="size-category">
                    ...
                    <div class="size-options">
                        <label><input type="checkbox" value="Foundation_Brush" onchange="toggleMultiSizeSelection('${productIndex}', 'Foundation_Brush')"> Foundation Brush</label>
                        <label><input type="checkbox" value="Concealer_Brush" onchange="toggleMultiSizeSelection('${productIndex}', 'Concealer_Brush')"> Concealer Brush</label>
                        <label><input type="checkbox" value="Eyeshadow_Brush" onchange="toggleMultiSizeSelection('${productIndex}', 'Eyeshadow_Brush')"> Eyeshadow Brush</label>
                        <label><input type="checkbox" value="Blush_Brush" onchange="toggleMultiSizeSelection('${productIndex}', 'Blush_Brush')"> Blush Brush</label>
                        <label><input type="checkbox" value="Lip_Brush" onchange="toggleMultiSizeSelection('${productIndex}', 'Lip_Brush')"> Lip Brush</label>
                        <label><input type="checkbox" value="Makeup_Remover" onchange="toggleMultiSizeSelection('${productIndex}', 'Makeup_Remover')"> Makeup Remover</label>
                        <label><input type="checkbox" value="Brush_Set" onchange="toggleMultiSizeSelection('${productIndex}', 'Brush_Set')"> Brush Set</label>
                    </div>
                </div>
                ...
                <div class="size-category">
                    ...
                    <div class="size-options">
                        <label><input type="checkbox" value="Travel_Hair" onchange="toggleMultiSizeSelection('${productIndex}', 'Travel_Hair')"> Travel Size (50ml)</label>
                        <label><input type="checkbox" value="Regular_Hair" onchange="toggleMultiSizeSelection('${productIndex}', 'Regular_Hair')"> Regular (250ml)</label>
                        <label><input type="checkbox" value="Large_Hair" onchange="toggleMultiSizeSelection('${productIndex}', 'Large_Hair')"> Large (500ml)</label>
                        <label><input type="checkbox" value="Family_Hair" onchange="toggleMultiSizeSelection('${productIndex}', 'Family_Hair')"> Family Size (1L+)</label>
                    </div>
                </div>
                ...
                <div class="size-category">
                    ...
                    <div class="size-options">
                        <label><input type="checkbox" value="Mini_Bath" onchange="toggleMultiSizeSelection('${productIndex}', 'Mini_Bath')"> Mini (50ml)</label>
                        <label><input type="checkbox" value="Regular_Bath" onchange="toggleMultiSizeSelection('${productIndex}', 'Regular_Bath')"> Regular (250ml)</label>
                        <label><input type="checkbox" value="Large_Bath" onchange="toggleMultiSizeSelection('${productIndex}', 'Large_Bath')"> Large (500ml)</label>
                        <label><input type="checkbox" value="Family_Bath" onchange="toggleMultiSizeSelection('${productIndex}', 'Family_Bath')"> Family Size (1L+)</label>
                    </div>
                </div>
            `;
        }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            // Only initialize multi-product forms if we're not editing a single product
            <?php if (!isset($product) || !$product): ?>
        ...
        // Clear forms on page load/refresh (only for new products)
        window.addEventListener('load', function() {
            // Only clear forms if we're not editing an existing product
            <?php if (!isset($product) || !$product): ?>
                clearAllForms();
            <?php endif; ?>
        });
